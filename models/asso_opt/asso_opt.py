...
class AssoConcept(pl.LightningModule):
    def init_weight_concept(self, concept2cls):
        self.init_weight = th.zeros(
            (self.cfg.num_cls, len(self.select_idx))
    ...
    def __init__(self, cfg, init_weight=None, select_idx=None) -> None:
        ...
        if not concept_feat_path.exists():
            ...
            if select_idx is None:
                self.select_idx = th.load(select_idx_path)[: cfg.num_concept]
            else:
                self.select_idx = select_idx

            self.concepts = th.load(concept_feat_path)[self.select_idx]
            ...
            self.concept2cls = (
                th.from_numpy(np.load(concept2cls_path)[self.select_idx])
                .long()
                .view(1, -1)
            )
        ...
        self.train_acc = torchmetrics.Accuracy(
            num_classes=cfg.num_cls, task="multiclass", num_labels=cfg.num_cls
        )
        ...
        self.confmat = torchmetrics.ConfusionMatrix(
            task="multiclass", num_classes=self.cfg.num_cls
        )
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        if self.cfg.asso_act == "softmax":
            row_l1_norm = th.linalg.vector_norm(
    ...
    def on_test_epoch_end(self):
        # torchmetrics v2: compute aggregated accuracy
        try:
            self.total_test_acc = self.test_acc.compute()
        except Exception:
            # fallback to manual compute from stored preds/targets
            all_y = th.hstack(self.all_y) if len(self.all_y) > 0 else None
            all_pred = th.hstack(self.all_pred) if len(self.all_pred) > 0 else None
            if all_y is not None and all_pred is not None:
                self.total_test_acc = (all_pred == all_y).float().mean()
            else:
                self.total_test_acc = th.tensor(0.0)
