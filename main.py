...
def linear_probe_sklearn_main(cfg):
    ...
    for seed in range(1, cfg.n_runs + 1):
        np.random.seed(seed)
        random.seed(seed)
        data_module = LinearProbeDataModule(
            cfg.data_root,
            cfg.bs,
            cfg.img_split_path,
            cfg.img_path,
        ...
        train_features, train_labels = get_features(
            data_module.train_dataloader(), cfg.paper, cfg.clip_model
        )
        val_features, val_labels = get_features(
            data_module.val_dataloader(), cfg.paper, cfg.clip_model
        )
        ...
        for c_weight in search_list:
            ...
            acc_val = (
                np.mean([int(t == p) for t, p in zip(val_labels, pred)]).astype(
                    np.float32
                )
                * 100.0
            )
        ...
        def binary_search(c_left, c_right, seed, step, val_acc_step_list):
            clf_left = LogisticRegression(  # random_state=0,
                C=c_left, max_iter=1000, verbose=1, n_jobs=8
            )
            clf_left.fit(train_features, train_labels)
            pred_left = clf_left.predict(val_features)
            accuracy = np.mean((val_labels == pred_left).astype(np.float32)) * 100.0
            acc_left = (
                np.mean([int(t == p) for t, p in zip(val_labels, pred_left)]).astype(
                    np.float32
                )
                * 100
            )
            ...
            acc_right = (
                np.mean([int(t == p) for t, p in zip(val_labels, pred_right)]).astype(
                    np.float32
                )
                * 100
            )
            ...
            saveline = "{}, seed {}, {} shot, weight {}, val_acc {:.2f}\n".format(
                cfg.dataset, seed, cfg.n_shots, c_final, val_acc
            )
            return (
                np.power(10, c_left),
                np.power(10, c_right),
    ...
    classifier = LogisticRegression(  # random_state=0,
        C=best_c, max_iter=1000, verbose=1, n_jobs=8
    )
    classifier.fit(train_features, train_labels)
    test_features, test_labels = get_features(
        data_module.test_dataloader(), cfg.paper, cfg.clip_model
    )
    ...
    print(
        "{} {}shot {}, best C = {}\n".format(
            cfg.dataset, cfg.n_shots, cfg.clip_model, best_c
        )
    )
    ...
    # test performance
    accuracy = np.mean((test_labels == predictions).astype(np.float32)) * 100.0
    print(f"Test Accuracy = {accuracy:.3f}")

    with open(
        "/Users/nam.le/Desktop/research-phd/LaBo/output/linear_probe/{}.txt".format(
            cfg.dataset
        ),
        "a",
    ) as f:
        f.write(
            "{} {}shot {}, best C = {}, seed = {}\n".format(
                cfg.dataset, cfg.n_shots, cfg.clip_model, best_c, seed
            )
        )
    ...
    print(
        "Macro Accuracy = {}".format(
            np.mean(matrix.diagonal() / matrix.sum(axis=1)) * 100
        )
    )
...
def asso_opt_main(cfg):
    ...
    from models.select_concept.select_algo import (
        mi_select,
        clip_score_select,
        group_mi_select,
        group_clip_select,
    ...
    try:
        print(cfg.submodular_weights)
    except:
        cfg.submodular_weights = "none"
    ...
    if cfg.proj_name == "ImageNet" and (cfg.n_shots == "all" or cfg.n_shots == 16):
        ...
        model = AssoConcept(
            cfg,
            init_weight=(
                th.load(cfg.init_weight_path) if "init_weight_path" in cfg else None
            ),
        )
    else:
        print("use asso concept with dot product loader, faster")
        model = AssoConceptFast(
            cfg,
            init_weight=(
                th.load(cfg.init_weight_path) if "init_weight_path" in cfg else None
            ),
    ...
    if cfg.proj_name == "ImageNet" and cfg.n_shots == "all":
        check_interval = 5
    else:
        check_interval = 10
    ...
    if not cfg.DEBUG:
        ...
        wandb_logger = pl.loggers.WandbLogger(
            name="{}shot_{}_{}_{}".format(
                cfg.n_shots,
                cfg.concept_type,
                cfg.num_concept,
                cfg.submodular_weights[1],
            ),
            project=proj_name,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--work-dir", help="work directory to save the ckpt and config file"
    )
    parser.add_argument("--func", help="which task to run")
    parser.add_argument(
        "--test", action="store_true", help="whether to enable test mode"
    )
    parser.add_argument(
